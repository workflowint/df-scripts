...
declare @TableNameOLD varchar(255)
...
while @@fetch_status = 0
    begin
		  if ( @TableNameOLD <> @TableName )	
		    begin
		    set @SQLString1 ='DROP FULLTEXT INDEX ON '+@TableName
		    EXEC(@SQLString1)
		    end
		  set @SQLString1 =' ALTER TABLE '+@TableName+' ALTER COLUMN '+@ColumnName+' nvarchar(max)  '
		  EXEC(@SQLString1)
		  if ( @TableNameOLD <> @TableName )	
			  set @SQLString1 =' CREATE FULLTEXT INDEX ON ' + @TableName + '(' + @ColumnName + ') KEY INDEX ' + @IndexName + 
			  ' ON ' + @CatalogName 
		  else	  
			  set @SQLString1 =' ALTER FULLTEXT INDEX ON ' + @TableName + ' ADD (' + @ColumnName + ') ' 
		  EXEC(@SQLString1)
