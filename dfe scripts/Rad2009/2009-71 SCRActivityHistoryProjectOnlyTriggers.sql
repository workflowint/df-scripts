/****** Object:  Trigger [dbo].[DeleteLinkObjectToActivityHistory]    Script Date: 09/04/2012 16:26:55 ******/
SET ANSI_NULLS ON
GO
    ...
    while @@fetch_status = 0
     begin
        ...
        if ( @PeUpdate > 0 and @Done = 0 )
		  begin
			set @Done = 1
     		UPDATE People set LASTAHID=A.ActivityHistoryID,LastAHCompletedOn=A.CompletedOn
            from People,
		    (Select ActivityHistoryID,LinkObjectToActivityHistory.LeftID,CompletedOn,
            Rank() over ( partition by LinkObjectToActivityHistory.LeftID order By CompletedOn desc) as dd
            from ActivityHistory WITH (NOLOCK)  
		    JOIN LinkObjectToActivityHistory WITH (NOLOCK) ON  
		    ActivityHistory.ActivityHistoryID=LinkObjectToActivityHistory.RightID
		    and LinkObjectToActivityHistory.ObjectTableName='People' ) as A
		    where People.PeopleID=A.LeftID and People.PeopleID = @PeopleID and dd =1
          end
        fetch next from CurLoc2 into @PeopleID,@ProjectsID, @PrUpdate, @PeUpdate
    end
   close         CurLoc2
...
GO
/****** Object:  Trigger [dbo].[LinkObjectToActivityHistoryInsert]    Script Date: 09/04/2012 16:27:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  Trigger [dbo].[DeleteActivityHistory]    Script Date: 09/04/2012 16:25:11 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER TRIGGER [dbo].[DeleteActivityHistory] ON [dbo].[ActivityHistory] 
FOR DELETE 
AS
SET NOCOUNT ON;
declare @PeopleID int
