ALTER TABLE People add GDPRDMarkedManually bit, CustomText6 nvarchar(255),CustomText7 nvarchar(255)
GO
ALTER TABLE GDPRLog add GDPRNote nvarchar(50)
GO

/****** Object:  Trigger [dbo].[PeopleDelete]    Script Date: 01/16/2020 10:36:13 ******/
SET ANSI_NULLS ON
GO
...
begin   
 ...
 begin
	IF (SELECT COUNT(*) FROM deleted where IsNull(PermissiontoRetainData,0)=0 and PermissiontoRetainDate >'01/01/2017') > 0 
		BEGIN 
		INSERT INTO GDPRLog ( PeopleID,FirstName, LastName, Phone1,Phone2,Phone3,Phone4,Phone5,EmailAddress, GDPRNote )
		SELECT PeopleID, FirstName, LastName, Phone1,Phone2,Phone3,Phone4,Phone5,dbo.fn_GetAllEmails(deleted.PeopleID),
		case when GDPRDMarkedManually =1 then 'Marked Manually' else null end 
